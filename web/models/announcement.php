	...
	public function save() {
		if($this->registry->getObject('auth')->isLoggedIn()) {
			...
			if ($this->id > 0) {
				...
				$data[] = $this->keywords;
				$data[] = $this->id;
				return $this->registry->getObject('db')->callRutine('editArticle', $data);
			}
			else {
				$data['id_user'] = $this->registry->getObject('auth')->getUser()->getId();
				$data['id_category'] = $this->categoryId;
				$data['article_title'] = $this->title;
				$data['article_text'] = $this->text;
				$data['article_keywords'] = $this->keywords;
				$data['article_date'] = date('Y-m-d H:i:s');
				$result = $this->registry->getObject('db')->insertRecords('articles', $data);
				//$result = $this->registry->getObject('db')->callRutine("insertArticle", $data);
